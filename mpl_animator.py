...
def scan_ast(src):
    ...
    for stmt in tree.body:
        ...
        info.node_has_funcanimation[stmt_id] = False
        ...
        for node in ast.walk(stmt):
            ...
            # Detect FuncAnimation calls (to be stripped)
            if isinstance(node, ast.Call):
                func = node.func
                if isinstance(func, ast.Name) and func.id == "FuncAnimation":
...
def partition(src, tree, info, dep_vars, var):
    ...
    # Collect axes variable names for routing dynamic-but-draws-on-axes stmts
    ax_var_names = {a.var_name for a in info.ax_info}
    ...
    for stmt in tree.body:
        ...
        # Strip existing FuncAnimation calls — we replace them with our own
        if info.node_has_funcanimation[sid]:
            continue
        ...
        if isinstance(stmt, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
            # Skip inner callback functions used by FuncAnimation (they
            # reference variables that will be moved into update())
            _func_name = stmt.name if hasattr(stmt, "name") else ""
            _is_anim_callback = False
            for other in tree.body:
                if info.node_has_funcanimation.get(id(other), False):
                    if _func_name and _func_name in _node_uses_names(other):
                        _is_anim_callback = True
                        break
            if _is_anim_callback:
                continue  # strip the callback too
        ...
        # Does this statement reference any axes variable? If so and it
        # depends on the animated var, it should go to plot_stmts (after
        # clear) rather than dynamic_stmts (before clear).
        uses_axes = bool(stmt_names & ax_var_names) if ax_var_names else False
        ...
        if not fig_found:
            ...
            if has_draw or has_config or uses_plot_assigned:
                ...
                if uses_axes:
                    # Depends on animated var AND touches axes — must run
                    # after clear, so route to plot_stmts
                    plot_stmts.append(text)
                    plot_assigned.update(assigned)
                else:
                    dynamic_stmts.append(text)
...
def _rewrite_wildcard_for_worker(stmts, used_names):
    """Rewrite `from matplotlib.pyplot import *` to explicit imports for worker body.

    Python forbids `import *` inside function bodies, so we expand it to
    `from matplotlib.pyplot import name1, name2, ...` using only the names
    actually referenced in the script.
    """
    # Common pyplot names that might appear as bare calls after `import *`
    _PYPLOT_NAMES = {
        "subplots", "subplot", "figure", "show", "savefig", "close",
        "plot", "scatter", "bar", "barh", "hist", "imshow", "contour",
        "contourf", "fill_between", "fill_betweenx", "errorbar",
        "pie", "boxplot", "violinplot", "stem", "step", "hexbin",
        "pcolormesh", "pcolor", "streamplot", "quiver", "barbs",
        "title", "xlabel", "ylabel", "xlim", "ylim", "legend",
        "grid", "tight_layout", "suptitle", "colorbar", "text",
        "annotate", "gca", "gcf", "clf", "cla", "axes",
        "subplot2grid", "subplot_mosaic", "margins",
        "xticks", "yticks", "xscale", "yscale",
        "loglog", "semilogx", "semilogy",
        "rcParams", "rc", "style",
        "sin", "cos", "sqrt", "pi",  # numpy-like names sometimes pulled in
    }
    needed = sorted(used_names & _PYPLOT_NAMES)
    result = []
    for stmt in stmts:
        if "from matplotlib.pyplot import *" in stmt:
            result.append("import matplotlib.pyplot as plt")
            if needed:
                result.append(f"from matplotlib.pyplot import {', '.join(needed)}")
        else:
            result.append(stmt)
    return result
...
def animate(src, var="t", range_str="0,1", frames=120, fps=25,
    ...
    # Warn if existing FuncAnimation was detected and stripped
    has_funcanimation = any(info.node_has_funcanimation.values())
    if has_funcanimation:
        warnings.warn(
            "Existing FuncAnimation detected and stripped. "
    ...
    # Build worker-specific static stmts: rewrite `from X import *` to
    # explicit imports since Python forbids `import *` inside functions.
    has_wildcard = info.pyplot_alias is None
    if has_wildcard:
        worker_static = _rewrite_wildcard_for_worker(final_static, info.all_names)
    else:
        worker_static = list(final_static)
